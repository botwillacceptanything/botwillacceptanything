...
module.exports = function (config, events) {
  ...
  app.get('/', function (req, res) {
    ...
    repo.commits(function (err, commits) {
      var commitLog = commits.map(function (commit) {
        return '<p><a href="/stdout">Standard Output</a></p><li>' +
            sanitizeAllHtml(commit.author.name + ' added ' + commit.id) +
            "<br />" +
            sanitizeAllHtml(commit.message).replace("\n", '<br />') +
          '</li>';
      });
      var response = '<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"/><style>li:nth-child(2n+1){background: #efffef;}li:nth-child(2n){background: #efefff;} </style><title>Recent commits</title></head><body>Last 10 commits:<br/><ul style="display: inline-block">';
  ...
  app.get('/stdout', function (req, res) {
    var response = '<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"/><style>li:nth-child(2n+1){background: #efffef;}li:nth-child(2n){background: #efefff;} </style><title>Standard Output</title></head><body><h1>Standard Output</h1><ul style="display: inline-block"><li>';

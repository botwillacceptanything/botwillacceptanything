(function() {
    ...
    define(deps, function(
        ...
        // A voting map must satisfy the following constraints:
        //
        // (0) a-b means a upvotes and b downvotes
        // (1) a policy mark is one of . L W
        // (5) a . mark means it is not possible to come to a decision yet
        // (6) marks L and W signify loss and win
        ...
        var canDecideVote = function (yeas, nays) {
            if (yeas < 0) { yeas = 0; }
            if (yeas > VOTE_SIZE) { yeas = VOTE_SIZE-1; }
            if (nays < 0) { nays = 0; }
            if (nays > VOTE_SIZE) { nays = VOTE_SIZE-1; }
            return ( VOTE_MAP[yeas][nays] != '.' );
        };
...
// because otherwise people can "react" to it by voting in the opposite direction (without sock-puppet accounts).
        ...
        function processPR(pr, cb) {
            ...
            // only make a decision if policy allows
            if (! canDecideVote(voteResults.positive,voteResults.nevative)) {

(function() {
    ...
    define(deps, function(
        ...
        var canDecideVote = function (yeas, nays) {
            return (Math.max(yeas, nays) >= GUARANTEED_RESULT);
        };
...
// because otherwise people can "react" to it by voting in the opposite direction (without sock-puppet accounts).
        ...
        function processPR(pr, cb) {
            ...
            if (! canDecideVote(voteResults.positive, voteResults.negative)) {
        ...
        voting.canDecideVote = canDecideVote;
        voting.decideVoteResult = decideVoteResult;

(function() {
    ...
    define(deps, function(
      ...
      var Voting = function Voting(repo) {
        ...
        this.repo = repo;
        this.votingParams = repo.votingParams;
        this.votingParams.truePeriod =
          this.votingParams.period * (1 + (Math.random() - 0.5) * this.votingParams.period_jitter);
        this.votingParams.guaranteedResult =
          Math.ceil(this.votingParams.minVotes * this.votingParams.supermajority);
        this.voteController =
          new VoteController(repo, this.votingParams, this.starGazers, this.votesPerPR);
      ...
      Voting.prototype.initialize = function initialize() {
        ...
        if (typeof config.githubAuth === 'undefined') {
          this.intervals.refreshAllPRs = setInterval(this.refreshAllPRs.bind(this), this.votingParams.pollInterval * MINUTE);
        } else {
        ...
        this.intervals.updateStarGazers = setInterval(this.updateStarGazers.bind(this), this.votingParams.pollInterval * MINUTE);
      };
      ...
      Voting.prototype.processPR = function processPR(pr, cb) {
        ...
        if (age / MINUTE < this.votingParams.truePeriod) { return cb(null, false); }
        ...
        if (highestVote < this.votingParams.guaranteedResult) { return cb(null, false); }

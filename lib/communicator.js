    ...
    */
    ...
    function instrument(string, words, person) {
        ...
        while((res = regex.exec(string)) != null) {
            ...
            if(key === 'person') {
                ...
                string += person;
                string += string.substr(index + group.length, string.length);
            }
    ...
    function randomNegativeConstruct(name) {
        ...
        var sentences = [
            ...
            'I hate you {{person}}. Better watch out or I will ' +
                '{{botAction}} you.',
            '{{person}} is a {{name}}.',
            '{{person}} you are {{attribute}} and no one likes you.',
            'Hello, {{person}}, looks like you didn\'t {{personAction}}' +
                ' yesterday. Too bad.'
        ];
    ...
    function greet(name) {
        /*
         * Based upon this score we select constructs until
        ...
        if(score <= -3) {
            // Only allow negative greetings if we're truly evil.
            if (config.evil !== true) {
                return undefined;
            }

            // Introduce additional negative greetings
            greetings = greetings.concat(['Ugh']);

            score *= -1;

            while(score > 0) {
                sentence = randomNegativeConstruct(name);
                string += sentence.string + ' ';
                score -= sentence.score;
                score--;
            }
        } else if(score >= 3){
            // Introduce additional friendly greetings
            greetings = greetings.concat(['Oh, hello']);

            while(score > 0) {
                sentence = randomPositiveConstruct(name);
                string += sentence.string + ' ';
                score -= sentence.score;
                score--;
            }
            ...
            while(score > 0) {
                sentence = randomNeutralConstruct(name);
                string += sentence.string + ' ';
                score -= sentence.score;
                score--;
            }
        ...
        return rd(greetings) + ' ' + name + ', ' + string;
    }

(function () {
    ...
    define(deps, function(events, Twitter) {
        ...
        events.on('github.pull_request.merged', function (event) {
            ...
            // Tweet PR merged
            module.exports.postTweet('I now have the ability to ' + pr.title + pr.html_url);
        });

        events.on('bot.pull_request.vote_started', function (pr) {
            // Tweet vote started
            module.exports.postTweet('Vote started for PR #' + pr.number + ': ' + pr.html_url);
        });

        events.on('github.pull_request.closed', function (event) {
            var pr = event.pull_request;
            // Tweet PR closed
            module.exports.postTweet('PR #' + pr.number + ' has been closed: ' + pr.html_url);
        });
    });

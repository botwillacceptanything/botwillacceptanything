(function() {
  'use strict';
	...
	function containsStr(string,lookfor) {
		var re = new RegExp(".*?" + lookfor + ".*?");
		return string.toLowerCase().match(re) !== null;
	}
  ...
  Conversation.prototype.sleepFor = function sleepFor(s) {
	...
	Conversation.prototype.react = function(word) {
		...
		if(word.toLowerCase().indexOf(this.nick.toLowerCase()) !== -1) {
			...
			else if(containsStr(word,'preempt')) {
				this.emit('say', 'ok, starting the warning playlist...');
				this.emit('preempt');
			}
			else if(containsStr(word,'usa')) {
				if(containsStr(word,'up')) {
					this.emit('usa','up');
					this.emit('say','ok, forming a government');
				}
				else if(containsStr(word,'down')) {
					this.emit('usa','down');
					this.emit('say','ok, starting a revolution');
				}
				else if(containsStr(word,'half')) {
					this.emit('usa','half');
					this.emit('say','ok, descending into 1984');
				}
				else if(containsStr(word,'dictator')) {
					this.emit('usa','dictator');
					this.emit('say','ok, you want it WHAT way?');
				}
				else if(containsStr(word,'friends')) {
					this.emit('usa','friends');
					this.emit('say','Yaaay :D');
				}
				else {
					this.emit('say',"you want me to do WHAT?!?!");
				}
			}
